Shader
{
	Properties
	{
		...
		Texture2D _MetallicMap = "black"
		Texture2D _RoughnessMap = "white"
		Texture2D _OcclusionMap = "white"
	}
	Pass
	{
		Blend One Zero
		...
		TEXTURE2D(_MetallicMap);	SAMPLER(_MetallicMap_Sampler);
		TEXTURE2D(_RoughnessMap);	SAMPLER(_RoughnessMap_Sampler);
		TEXTURE2D(_OcclusionMap);	SAMPLER(_OcclusionMap_Sampler);

		inline void InitializeSurfaceData(float2 uv, out SurfaceData outSurfaceData)
		{
			outSurfaceData = (SurfaceData)0;
			float4 color = SAMPLE_TEXTURE2D(_BaseMap, _BaseMap_Sampler, uv);
			...
			outSurfaceData.alpha = 1;
#endif
			outSurfaceData.metallic = SAMPLE_TEXTURE2D(_MetallicMap, _MetallicMap_Sampler, uv).rgb;
			outSurfaceData.roughness = SAMPLE_TEXTURE2D(_RoughnessMap, _RoughnessMap_Sampler, uv).r;
			outSurfaceData.normalTS = SAMPLE_TEXTURE2D(_NormalMap, _NormalMap_Sampler, uv);
			outSurfaceData.emission = 0;
			outSurfaceData.occlusion = SAMPLE_TEXTURE2D(_OcclusionMap, _OcclusionMap_Sampler, uv).r;
		}
...
#if (TRANSPARENT)
			clip(_BaseMap.Sample(_BaseMap_Sampler, uv).a - 0.5);
#endif		

			//float3 normal = SAMPLE_TEXTURE2D(_NormalMap, _NormalMap_Sampler, uv).xyz;
			//float3 normalWS = NormalTSToNormalWS(normalize(normal), input.normalWS, input.tangentWS, input.bitangentWS);
			//return float4(1, 0, 0, 0);//return float4(normalWS * 0.5 + 0.5, 1.0f);
		}
		HLSLEND
	}
	Pass
	{
		...
		{
#if (TRANSPARENT)
			clip(SAMPLE_TEXTURE2D(_BaseMap, _BaseMap_Sampler, input.texcoord).a - 0.5);
#endif		
		}
		HLSLEND
	}
